...
import { Pose3DEditor } from "./vnccs_character_debugger_3d.js";
...
function ensurePoseEditorStyles() {
    if (document.getElementById("vnccs-pose-debugger-style")) {
        return;
    }

    const style = document.createElement("style");
    style.id = "vnccs-pose-debugger-style";
    style.textContent = `
...
class PoseEditorDialog {
    ...
    createThreeColumn() {
        ...
        const exportDebugBtn = this.create3DButton("Export Debug", () => {
            this.ensurePose3D().then((editor) => editor?.exportDebugPoses());
        });
        ...
        this.threeToolbar.appendChild(exportDebugBtn);
...
app.registerExtension({
    name: "VNCCS.CharacterDebugger",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "VNCCS_CharacterDebugger") {
            return;
        }
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            // Re-use PoseEditorManager but with our specialized dialog if needed, 
            // OR just use standard one because 3D editor logic is inside the 3D module we monkey-patched.
            // Result: The PoseEditorDialog imports Pose3DEditor from THIS file's imports.
            // Since we changed the import at the top of THIS file to point to ..._3d.js,
            // and PoseEditorDialog is defined in THIS file (conceptually, or imported?),
            // Wait, PoseEditorDialog is defined in THIS file (lines 835+).
            // It calls `Pose3DEditor.create(...)`.
            // So if we updated the import `import { Pose3DEditor } from "./vnccs_character_debugger_3d.js";`,
            // then PoseEditorDialog will use our new debug 3D editor. Perfect.

            const button = this.addWidget("button", "Open Debugger (3D)", null, () => {
